...
type serverUDPFlow struct {
	...
	endpoint  udpwire.Endpoint
	sessionID string
	// entry is the ring entry pinned by this flow's peer at handshake time
	// (ProofKit fork — there is no server-wide cipher). Replies encrypt
...
func (s *Server) handleDatagram(peerID string, ciphertext []byte) {
	...
	if entry == nil {
		// A datagram can only follow the TCP handshake that pins the peer's
		// key (ProofKit multi-key fork) — before that it cannot belong to an
		// authenticated peer, so there is nothing to trial-decrypt against.
		logger.Debugf("drop udp datagram before key pairing")
		return
	}
	wire, err := entry.Cipher.Decrypt(ciphertext)
	if err != nil {
...
// sendUDPFrame encrypts frame under entry — the ring entry the flow's peer
// pinned (ProofKit fork; there is no server-wide cipher) — and sends it back
...
func (s *Server) sendUDPFrame(peerID string, entry *crypto.RingEntry, frame udpwire.Frame) bool {
	...
	enc, err := entry.Cipher.Encrypt(wire)
	if err != nil {
...
func (s *Server) finishUDPFlow(flow *serverUDPFlow) {
	flow.closeOnce.Do(func() {
		...
		// ProofKit metering: attribute the flow's bytes to the session's key,
		// mirroring the TCP stream path (up = client→internet = bytesIn).
		if s.meter != nil {
...
// pinnedDatagramEntry returns the ring entry pinned by peerID's pairing group
// ("" = the single-link group), or nil while pairing is still open. Datagrams
// always follow the TCP handshake that pins the key, so nil means the sender
// is not an authenticated peer (ProofKit multi-key fork — replaces the
// removed server-wide cipher).
